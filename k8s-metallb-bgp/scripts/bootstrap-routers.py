...
Usage:
    ./bootstrap-routers.py [--server http://127.0.0.1:3080] [--routers R1 R2]
"""
...
def get_consoles(server):
    ...
    project = next((p for p in projects if p.get("status") == "opened"), None)
    if project is None and len(projects) == 1:
        project = projects[0]
    if project is None:
        sys.exit(f"no open GNS3 project on {server}; open the lab project first "
                 f"(found: {[p['name'] for p in projects]})")
    out = {}
    for n in api(f"/projects/{project['project_id']}/nodes"):
        if n["name"] in MGMT_IP:
            out[n["name"]] = (n.get("console_host") or "127.0.0.1", n["console"])
    return out
...
def main():
    ...
    ap.add_argument("--server", default="http://127.0.0.1:3080")
