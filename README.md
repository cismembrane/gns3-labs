...
| --- | --- | --- | --- |
| [k8s-metallb-bgp](k8s-metallb-bgp) | Single-node k3s cluster joined to the four-router eBGP ring as AS 65100. MetalLB peers with R1 and R4 and advertises each Kubernetes LoadBalancer service as a /32 host route, so service VIPs appear in `show ip bgp` on real IOS. Dual-homed peering means VIP reachability survives losing either edge router. Ansible configures the routers; Helm deploys MetalLB. | Kubernetes (k3s), MetalLB, Helm, eBGP, kube-proxy, Ansible | |
